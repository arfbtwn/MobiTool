...
import format.headers.Enumerations.MobiType;
...
public class MobiDocHeader implements ByteFieldContainer {
    ...
    public MobiType getType() {
        return MobiType.valueOf(fields.<IntByteField>getAs(MOBI_TYPE).getValue());
    }
    ...
    public void setExthHeader(boolean enable) {
        if (enable && null == exth) {
        	fields.<IntByteField>getAs(MINIMUM_VERSION_COMPATIBLE_READER).setValue(3);
